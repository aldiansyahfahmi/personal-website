    ...
    <div class="glow-card bg-dark-surface border border-primary/20 rounded-2xl p-8">
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                @if($project->image)
                <div class="mb-4">
                    <img src="{{ Str::startsWith($project->image, 'http') ? $project->image : (file_exists(public_path($project->image)) ? asset($project->image) : asset('storage/' . $project->image)) }}" class="w-40 h-24 object-cover rounded-lg border border-primary/20">
